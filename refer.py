...
class REFER:
	...
	def __init__(self, data_root, dataset='refcoco', splitBy='unc'):
		...
		if dataset in ['refcoco', 'refcoco+', 'refcocog']:
			self.IMAGE_DIR = osp.join(data_root, 'mscoco/train2014')
	...
	def getAnnIds(self, image_ids=[], cat_ids=[], ref_ids=[]):
		...
		if len(image_ids) == len(cat_ids) == len(ref_ids) == 0:
			...
			if ref_ids:
				ann_ids = list(set(ann_ids).intersection(set([self.Refs[ref_id]['ann_id'] for ref_id in ref_ids])))
		return ann_ids
	...
	def loadAnns(self, ann_ids=[]):
		...
		elif isinstance(ann_ids, (int, str)):
...
if __name__ == '__main__':
	refer = REFER(data_root='data', dataset='refcoco', splitBy='google')
